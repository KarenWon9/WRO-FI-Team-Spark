...
#include <Wire.h> // Library for the communication I2C.
...
#define IN1 4 // Define the control motor pin IN1.

#define IN2 5 // Define the control motor pin IN2.
...
#define MAXLEFT 156 //Higher value toward the left to the servomotor.

#define MAXRIGHT 5 // Higher value toward the right to the servomotor.

#define MAXSPEED 250 // Maximum speed of the motor.
...
#define LMIN 10 // Lower limit of the sensors reading.

#define LMAX 10 // Maximum limit of the sensors reading.
...
unsigned long encodeCounter = 0; // Count to the encoder pulls.
...
void followLine(byte speed) {
  ...
  int grade = 0; // Define the variable 'grade' which represents the correction angle.

  if (DEBUG2) Serial3.println("followLine");

  readDistance(currentPosition); // Read the distances and upload 'currentPosition'

  if (currentPosition[0] < 10) { // If the frontal distance is less to 10 millimeters
  ...
  if (tempPosition[2] < tempPosition[3]) { // If the frontal distance is less to the left
    ...
    grade = map(currentPosition[2], tempPosition[2] - LMIN, tempPosition[2] + LMIN, TURNGRADE, -TURNGRADE); // Map the correction angle to the right.   
 ...
 if (DEBUG) Serial3.println("1 Actual: " + String(currentPosition[2]) + ". Inicial: " + String(tempPosition[2]));
    ...
    grade = map(currentPosition[3], tempPosition[3] - LMIN, tempPosition[3] + LMIN, -TURNGRADE, TURNGRADE); // Map the correction angle to the left.    if (DEBUG) Serial3.println("2 Actual: " + String(currentPosition[3]) + ". Inicial: " + String(tempPosition[3]));
  ...
  turnGrade(grade); // Turn the servomotor to the calculated angle

  turnGrade(grade); // Repeat the action to assure precision.

  moveCM(2, 1, speed); // Move the robot 2 cm forward at the specified speed
...
void goCar() {
  ...
  bool endCycle = false; // Define a variable to the block detection.
...
azul:
          ...
          do {
            ...
            followLine(MAXSPEED); // Follow the inner walls at the highest speed.
...
orange:
          ...
          do {
            ...
            followLine(MAXSPEED); // Follow the inner Wall to a higher speed.
      ...
      followLine(MAXSPEED); // Follow the inner wall at a higher speed.   
...
void setup() {
  ...
  center(); // Centra el servomotor

  doceVueltas(); // Call the 'doceVueltas' function to make 12 laps (three laps done)

  moveCM(20, 1, MAXSPEED); // Call the 'moveCM' function to move the robot 20 cm forward at the highest speed.
...
void doceVueltas() {
  ...
  for (count = 0; count < 12; count++) {
    ...
    followLine(MAXSPEED); // Call the 'followLine' function to follow the inner wall at the highest speed
  ...
  stop(); // Stop the robot after 12 completed laps.
